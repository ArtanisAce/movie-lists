...
export const getFilmDetails = state => {
  ...
  const searchResultsCached = state.searchResults.find(movie => movie.id === state.filmToShow);
  const cachedFilm = searchResultsCached ? searchResultsCached : state.filmsList.find(
